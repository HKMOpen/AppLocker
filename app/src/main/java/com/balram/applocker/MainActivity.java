...
public class MainActivity extends LockActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if (view.equals(btOnOff)) {
            ...
            Locker.Builder build = new Locker.Builder(intent)
                    .smartLock()
                    .withIcon(R.drawable.cardmen)
                    .withSymbol("㊙")

                    ;
            startActivityForResult(build.toBundle(), build.getType());
        } else if (view.equals(btChange)) {
            Intent intent = new Intent(this, LockActivity.class);
            Locker.Builder build = new Locker.Builder(intent)
                    .changePass(getString(R.string.enter_old_passcode))
                    .withPressFunc("QR", EMPTY_A.class)
